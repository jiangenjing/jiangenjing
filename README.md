# Hi there 👋 I'm Enjing Jiang

I'm a **master's student at Tsinghua Shenzhen International Graduate School (2025 intake)**, studying Logistics Engineering and Management. My research focuses on **time-series foundation models and time-series forecasting**, with an interest in connecting forecasting, optimization, and AI agents to real-world decisions.

![Tsinghua SIGS](https://img.shields.io/badge/Tsinghua-SIGS-6B2C91?style=flat-square)
![Research](https://img.shields.io/badge/Research-Time--Series%20Foundation%20Models-3155A4?style=flat-square)
![Forecasting and Optimization](https://img.shields.io/badge/Focus-Forecasting%20%26%20Optimization-187568?style=flat-square)

- 🔬 **Currently exploring** forecasting-driven inventory and warehouse allocation for cross-border e-commerce, alongside AI agents for explaining optimization decisions.
- 🌉 **UC Berkeley · Spring 2024:** a full-scholarship semester exchange student in the Berkeley Global Access program.
- 🌏 **Dreame Technology · Summer 2026:** overseas GTM intern in the AI Smart Ring business unit, working on market and competitor analysis, value-chain modeling, and pricing scenarios.
- 🎓 **Zhejiang University of Technology · 2025:** earned my bachelor's degree in Industrial Engineering at Jianxing Honors College, and received the **Zhejiang Outstanding Graduate** honor.
- 🏆 **CUMCM 2023 · First Prize, Zhejiang Division:** team leader and modeler for Problem C, connecting fresh-produce sales analysis, forecasting, replenishment, and pricing optimization.

## Selected research

My research experience spans predictive modeling, operations research, and data-driven energy policy.

- **Energy Policy · 2026** — [On-capacity vs. On-grid: Effect of subsidy on the adoption of high-quality energy storage facilities in the renewable energy industry](https://doi.org/10.1016/j.enpol.2025.114922). Co-author; evolutionary-game analysis of storage quality and subsidy design. [Project →](https://github.com/jiangenjing/energy-storage-subsidy-game)
- **PLOS ONE · 2025** — [Integrating DRN-RF with computer vision for detection of control room operator's mental fatigue](https://doi.org/10.1371/journal.pone.0320780). Co-author; contributed to data curation, project administration, visualization, and manuscript preparation. [Research showcase →](https://github.com/jiangenjing/drn-rf-fatigue-research)
- **Energy Strategy Reviews · 2026** — [Data-driven complementary policy development for photovoltaic industry facing resource constraints: A case study of Zhejiang Province in China](https://doi.org/10.1016/j.esr.2026.102128). Co-corresponding author; regional classification and complementary photovoltaic development. [Research showcase →](https://github.com/jiangenjing/photovoltaic-complementary-policy)

## Selected projects

| Project | What it explores |
| --- | --- |
| [🥬 CUMCM 2023 · Vegetable Replenishment & Pricing](https://github.com/jiangenjing/cumcm-2023-c-vegetable-replenishment) | Sales analysis, demand forecasting, and joint replenishment–pricing optimization; team paper and original experiments. |
| [📦 Fresh-Produce 3D Bin Packing](https://github.com/jiangenjing/fresh-produce-3d-bin-packing) · [Demo](https://jiangenjing.github.io/fresh-produce-3d-bin-packing/) | Undergraduate thesis on packing algorithms, with source materials and an interactive illustration. |
| [⚡ Shell × Hangzhou · AI Energy Brain](https://github.com/jiangenjing/shell-hangzhou-ai-energy-brain) | An energy-competition prototype exploring charging recommendations, station operations, and membership services. |

*Research showcases illustrate the methods and findings; they are not claims of complete experimental reproduction. The energy-competition prototype is a student project, not an official Shell product.*

---

**Interests:** Time-Series Foundation Models · Forecasting · Optimization · AI Agents  
**Tools I work with:** Python · Pandas · NumPy · scikit-learn · Gurobi · LangGraph
